...
let notElectedParties = enhancedParties.filter(party => party.seats !== undefined && party.seats === 0);
let notElectedPartyBunches = getNotElectedPartyBunches(notElectedParties);
let electedPartyBunches = getElectedPartyBunches(enhancedParties, notElectedParties);
...
<div class="s-q-item q-election-seats" style="opacity: 0;">
  ...
  <div id="q-election-seats-svg-container" class="q-election-seats-svg-container" />
  ...
  <Footer notes={item.notes} sources={item.sources} updatedDate={item.updatedDate} {hideUpdatedDate} />
</div>
